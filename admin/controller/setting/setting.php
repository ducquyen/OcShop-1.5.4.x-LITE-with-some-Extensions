...
class ControllerSettingSetting extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->post['config_contact'])) {
			$this->data['config_contact'] = $this->request->post['config_contact'];
		} else {
			$this->data['config_contact'] = $this->config->get('config_contact');			
		}

		if ($this->config->get('config_contact') && file_exists(DIR_IMAGE . $this->config->get('config_contact')) && is_file(DIR_IMAGE . $this->config->get('config_contact'))) {
			$this->data['contact'] = $this->model_tool_image->resize($this->config->get('config_contact'), 100, 100);		
		} else {
			$this->data['contact'] = $this->model_tool_image->resize('no_image.jpg', 100, 100);
		}
		//ocshop
